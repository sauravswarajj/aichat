...
export function Sidebar({ isOpenMobile = false, onCloseMobile }: SidebarProps) {
        ...
        >
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <div className="truncate text-xs">
              <span className="block truncate font-medium">{t.title || "New Chat"}</span>
              {t.preview && (
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "w-[280px] shrink-0 bg-[var(--bg-sidebar)] border-r border-[var(--border-subtle)] flex flex-col h-full z-40 transition-transform duration-200 lg:translate-x-0 fixed lg:static top-0 bottom-0 left-0",
          isOpenMobile ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
        )}
      >
        {/* Workspace Header (From Stitch Design) */}
        <div className="p-4 border-b border-[var(--border-subtle)]">
          <div className="flex items-center justify-between mb-3">
            ...
            {isOpenMobile && (
              <button
                onClick={onCloseMobile}
                className="lg:hidden p-1.5 text-[var(--text-muted)] hover:text-[var(--text-primary)] rounded hover:bg-[var(--bg-hover)]"
              >
                <X className="w-4 h-4" />
              </button>
            )}
